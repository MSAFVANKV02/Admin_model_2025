...
import {
  ...
  delete_Product_Api,
  get_All_Purchased_Products_Api,
...
export const getAllProductsInAdmin = async (
  ...
  try {
    ...
    const route =
      url === "deleted"
        ? await get_Deleted_Product_Api()
        : url === "purchase"
        ? await get_All_Purchased_Products_Api()
        : await get_Products_Api(filter);
    ...
    if (status === 200 || status === 201) {
      ...
      return {
        status: status,
        data: url === "deleted" || url === "purchase" ? data.data : data.products,
...
export const DeleteProductFn = () => {
  ...
  // 2. hard delete all categories ======
  const hardDeleteSingleProductFn = async (data: {
    productId: string;
    hardDelete: boolean;
  }) => {
    try {
