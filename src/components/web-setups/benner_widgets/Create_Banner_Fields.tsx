...
const validationSchema = Yup.object().shape({
  bannerImages: Yup.array().of(
    Yup.object().shape({
      ...
      //     .matches(
      //       /^https:\/\/ayaboo\.com\//,
      //       "Redirect URL must start with https://ayaboo.com/"
      //     ),
      context: Yup.string().when(["banner_type"], ([banner_type], schema) => {
        return banner_type === "ads"
          ? schema.required("Context is required for ads")
          : schema;
      }),
                  ...
                  {newBanner === "ads" && (
                    <div className=" flex justify-between gap-1 ">
                      ...
                      <div className="flex flex-col gap-3 w-3/4">
                        ...
                        <ErrorMessage
                          name={`bannerImages[${index}].context`}
                          component="span"
                          className="text-red-500 text-xs"
                        />
                      </div>
...
export default CreateBannerFields;
