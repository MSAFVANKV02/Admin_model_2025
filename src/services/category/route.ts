import {
  CREATE_CATEGORY_URL,
  GET_CATEGORY_URL,
  GET_CATEGORY_WITH_SUB_URL,
  HARD_DELETE_ALL_CATEGORY_URL,
  HARD_DELETE_SINGLE_CATEGORY_URL,
...
export const toggle_Category_Api = async (
  field: "featured" | "published",
  id: string
) =>
  await API.patch(
    `${TOGGLE_CATEGORY_URL}/${id}`,
    { field },
    { withCredentials: true }
  );
...
export const hard_Delete_Single_Category_Api = async (id: string) =>
  await API.delete(`${HARD_DELETE_SINGLE_CATEGORY_URL}/${id}`, {
    withCredentials: true,
  });

// 5.  update category

export const update_Category_Api = async (
  data: {
    name: string | null;
    parentId: string | null;
    coverImage: string | null;
  ...
  await API.put(`${UPDATE_CATEGORY_URL}/${id}`, data, {
    withCredentials: true,
  });
