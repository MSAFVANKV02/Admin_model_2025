...
// 5. Delete product
export const delete_Product_Api = async (data: {
  productId?: string;
  hardDelete?: boolean;
}) =>
...
export const restore_Deleted_Product_Api = async (id?: string) =>
  await API.put(
    `${RESTORE_DELETED_PRODUCT_URL}/${id}`,
    {},
    {
      withCredentials: true,
    }
...
// 9. all purcahased products
export const get_All_Purchased_Products_Api = () => {
  return API.get(GET_ALL_PURCHASED_PRODUCT_URL, {
    withCredentials: true,
  });
};
